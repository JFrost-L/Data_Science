...
class SwinTransformer(nn.Module):
    def __init__(self, num_classes, model_name='swin_base_patch4_window7_224'):
        ...
        self.swin_transformer = timm.create_model(model_name, pretrained=True)
...
class ImageDataLoader:
    def __init__(self):
        ...
        train_transforms = transforms.Compose([
            ...
            transforms.RandomAffine(degrees=30, translate=(0.1, 0.1), scale=(0.8, 0.8), shear=10),
...
class ModelTrainer:
    def __init__(self, model, device, train_loader, val_loader, test_loader, mean, std, epochs=1, learning_rate=0.00001, label_smoothing=0.1):
        self.device = torch.device(device)
        self.model = model.to(self.device)
        self.train_loader = train_loader
        self.val_loader = val_loader
        self.test_loader = test_loader
        self.criterion = LabelSmoothingLoss(num_classes=3, smoothing=label_smoothing)
        ...
        self.best_model_path = None  # 최고 성능 모델의 경로 저장
        self.num_classes = 3
    ...
    def save_best_model(self, accuracy):
        ...
        model_save_path = f'swinTransformer_best_model_{accuracy:.4f}.pth'
        torch.save(self.model.state_dict(), model_save_path)
        print(f"Model saved at {model_save_path}")
        self.best_model_path = model_save_path  # 저장된 모델 경로 저장
        ...
        for epoch in range(self.epochs):
            ...
            with tqdm(total=len(self.train_loader), desc=f"Epoch {epoch + 1}/{self.epochs}", unit="batch", leave=False) as pbar:
                for features, labels in self.train_loader:
                    ...
                    outputs = outputs.view(labels.shape[0], self.num_classes, -1).mean(dim=2)
            ...
            # Validation accuracy is shown in percentage
